...
public class RCObject extends RCBase {
	...
	public int getBytes(byte[] dest, int pointer) {
		...
		pointer = SerializationUtils.writeBytes(dest, pointer, fieldCount);
		for (RCField field : fields)
			pointer = field.getBytes(dest, pointer);

		pointer = SerializationUtils.writeBytes(dest, pointer, stringCount);
		for (RCString string : strings)
			pointer = string.getBytes(dest, pointer);

		pointer = SerializationUtils.writeBytes(dest, pointer, arrayCount);
		for (RCArray array : arrays)
	...
	public static RCObject Deserialize(byte[] data, int pointer) {
		...
		result.fieldCount = SerializationUtils.readShort(data, pointer);
		pointer += 2;
		...
		result.stringCount = SerializationUtils.readShort(data, pointer);
		pointer += 2;
		...
		result.arrayCount = SerializationUtils.readShort(data, pointer);
		pointer += 2;
